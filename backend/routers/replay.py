...
def build_command_history(conn, cmd_rows) -> list[dict]:
    """Joins each command row to its certificate/verification/pipeline-step history.
    Shared by /replay and the Evidence Package endpoint (backend/routers/mission_analytics.py)
    so neither reimplements this join — one real query shape, reused."""
    history = []
    for row in cmd_rows:
...
@router.get("/replay")
def replay(limit: int = 50, mission_id: int | None = None):
    with engine.connect() as conn:
        ...
        if mission_id is not None:
            query = query.where(commands.c.mission_id == mission_id)
        cmd_rows = conn.execute(query.order_by(commands.c.id).limit(limit)).fetchall()
        return build_command_history(conn, cmd_rows)
